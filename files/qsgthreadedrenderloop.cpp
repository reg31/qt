...
void QSGRenderThread::processEvent(QSGRenderThreadEvent &e)
{
    ...
    [&](WMTryReleaseEvent &e) {
        ...
        if (!window || e.inDestructor) {
            ...
            Q_ASSERT_X(!e.inDestructor || !active.load(std::memory_order_relaxed),
    ...
    [&](WMGrabEvent &e) {
        ...
        if (rhi) {
            ...
            const auto handleGrabFrameOpFailure = [this, cd, &e](QRhi::FrameOpResult result) {
                ...
                if (beginFrameResult == QRhi::FrameOpSuccess) {
                    ...
                    *e.image = QSGRhiSupport::instance()->grabAndBlockInCurrentFrame(rhi, cd->swapchain->currentFrameCommandBuffer());
                    ...
                    if (result == QRhi::FrameOpSuccess) {
                        ...
                        *e.image = {};
                        handleGrabFrameOpFailure(result);
                    }
                } else {
                    handleGrabFrameOpFailure(beginFrameResult);
                }
            } else {
...
void QSGRenderThread::sync()
{
    ...
    const bool canSync = rhi && !windowSize.isEmpty() && (!d->rhi || d->swapchain);
...
void QSGRenderThread::syncAndRender()
{
    ...
    if (profileFrame) {
        ...
        if (!afterSyncTime)
            afterSyncTime = afterBeginFrameTime;
        if (!afterRenderTime)
            afterRenderTime = afterSyncTime;
        ...
        qCDebug(QSG_LOG_TIME_RENDERLOOP,
                ...
                static_cast<void *>(window),
                nsecsToMillis(afterSyncTime - afterBeginFrameTime),
                nsecsToMillis(afterSwapchainTime),
...
void QSGRenderThread::ensureRhiDevice()
{
    ...
    if (rhi) {
        ...
        if (profileRhiInit) {
            ...
            qCDebug(QSG_LOG_TIME_RENDERLOOP,
                    "[window %p][render thread] RHI warm-up: createRhi=%d ms, rhiSetup=%d ms, renderContext=%d ms, total=%d ms",
                    static_cast<void *>(window),
                    nsecsToMillis(createRhiTime),
                    nsecsToMillis(rhiSetupTime - createRhiTime),
                    nsecsToMillis(totalTime - rhiSetupTime),
                    nsecsToMillis(totalTime));
        }
...
void QSGRenderThread::ensureRhi()
{
    ...
    if (rhi && cd->swapchain && sgrc->rhi()) [[likely]]
        return;
    ...
    if (!sgrc->rhi() && !pixelSize.isEmpty()) [[unlikely]] {
        rhi->makeThreadLocalNativeContextCurrent();
        QSGDefaultRenderContext::InitParams params;
        params.rhi = rhi;
        params.sampleCount = rhiSampleCount;
...
{
#ifdef Q_OS_ANDROID
    // Remove event filter and associated code once qtbase change 735089 is merged.
...
void QSGThreadedRenderLoop::handleObscurity(Window *w)
{
    ...
    if (w->thread->isRunning()) {
        ...
        w->thread->postEvent(WMObscureEvent{});
    }
    startOrStopAnimationTimer();
}
...
{
    if (!QQuickWindowPrivate::get(window)->updatesEnabled || !window->isExposed())
        return;

    Window *w = windowFor(window);
    if (!w)
        return;
    if (!w->thread->isRunning() || !w->thread->active.load(std::memory_order_relaxed))
        return;
...
{
    if (!QCoreApplication::instance() || !w || !w->thread->isRunning()
            || !w->thread->active.load(std::memory_order_relaxed))
        return;
...
void QSGThreadedRenderLoop::releaseResources(Window *w, bool inDestructor)
{
    ...
    if (w->thread->isRunning() && w->thread->active.load(std::memory_order_relaxed)) {
        ...
        if (!w->thread->active.load(std::memory_order_relaxed)) {
...
void QSGThreadedRenderLoop::polishAndSync(Window *w, bool inExpose)
{
    ...
    if (w->actualWindowFormat.swapInterval() != 0 && sg->isVSyncDependent(m_animation_driver)) {
        ...
        if (isOutlier) {
            ...
            ++w->psTimeSampleCount;
            const int sampleLength = w->badVSync ? PS_RECOVERY_SAMPLE_LENGTH : PS_TIME_SAMPLE_LENGTH;
            if (w->psTimeSampleCount > sampleLength) [[unlikely]] {
                ...
                } else if (t >= vsyncRate * PS_RECOVERY_THRESHOLD) {
                    w->badVSync = false;
    ...
    w = windowFor(window);
    if (!canSyncWindow()) {
        qCDebug(QSG_LOG_RENDERLOOP, "- removed after afterAnimating, abort");
        Q_QUICK_SG_PROFILE_SKIP(QQuickProfiler::SceneGraphPolishAndSync,
                                QQuickProfiler::SceneGraphPolishAndSyncPolish, 2);
        Q_QUICK_SG_PROFILE_END(QQuickProfiler::SceneGraphPolishAndSync,
                               QQuickProfiler::SceneGraphPolishAndSyncAnimations);
    ...
    if (profileFrames) {
        qCDebug(QSG_LOG_TIME_RENDERLOOP, "[window %p][gui thread] Frame prepared, polish=%d ms, lock=%d ms, sync=%d ms, animations=%d ms",
                static_cast<void *>(window),
                nsecsToMillis(polishTime),
                nsecsToMillis(waitTime - polishTime),
                nsecsToMillis(syncTime - waitTime),
                nsecsToMillis(timer.nsecsElapsed() - syncTime));
    }
...
{
    Window *w = windowFor(window);
    if (w && w->thread && w->thread->isRunning() && w->thread->active.load(std::memory_order_relaxed)
            && !w->thread->surfaceAboutToBeDestroyed.load(std::memory_order_acquire))
