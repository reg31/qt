...
public:
    ...
    AllocatorPage()
        ...
        , allocated(PageSize)
    {
        std::iota(std::begin(blocks), std::end(blocks), 0);
        std::ranges::fill(data, '\0');
    }

    const Type *at(uint index) const
    {
        return reinterpret_cast<const Type *>(&data[index * sizeof(Type)]);
    }

    Type *at(uint index)
    {
        return reinterpret_cast<Type *>(&data[index * sizeof(Type)]);
    }
};
...
public:
    ...
    Type *allocate()
    {
        AllocatorPage<Type, PageSize> *p = nullptr;
    ...
    void release(Type *t)
    {
        ...
        for (int i=0; i<pages.size(); ++i) {
            ...
            if (reinterpret_cast<const Type *>(&p->data[0]) <= t
                    && reinterpret_cast<const Type *>(&p->data[PageSize * sizeof(Type)]) > t) {
                pageIndex = i;
                break;
            }
        }
        ...
        AllocatorPage<Type, PageSize> *page = pages.at(pageIndex);
        int index = static_cast<int>(reinterpret_cast<const char *>(t) - page->data)
...
struct Batch
{
    ...
    mutable uint uploadedThisFrame : 1;
...
Batch *Renderer::newBatch()
{
    ...
    if (size) {
        ...
        memset(&b->vbo, 0, sizeof(Buffer) * 2);
        b->ubuf = nullptr;
