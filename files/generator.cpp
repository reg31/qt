...
void Generator::addFunctions(const QList<FunctionDef> &list, const char *functype)
{
    for (const FunctionDef &f : list) {
        ...
        for (const auto &argument : f.arguments) {
            moc_print(out, "{}{}", comma, disambiguatedTypeName(argument.type.name));
            comma = ", ";
        }
...
void Generator::generateTypeInfo(const QByteArray &typeName, bool allowEmptyName)
{
    ...
    if (idx == -1) moc_print(out, "0x{:08x} | 0xFFFFu", IsUnresolvedType);
    else moc_print(out, "0x{:08x} | uint16_t({})", IsUnresolvedType, idx);
}
...
void Generator::addProperties()
{
    for (const PropertyDef &p : std::as_const(cdef->propertyList)) {
        ...
        if (notifyId != -1 || p.revision > 0) {
            moc_print(out, ", ");
            if (p.notifyId < -1) moc_print(out, "{:#x} | uint32_t({})", IsUnresolvedSignal, int(strings.indexOf(p.notify)));
...
void Generator::addEnums()
{
    for (const EnumDef &e : std::as_const(cdef->enumList)) {
        ...
        if (e.flags) {
            bool first = true;
            if (e.flags & EnumIsFlag) { moc_print(out, " QMC::EnumIsFlag"); first = false; }
            if (e.flags & EnumIsScoped) moc_print(out, "{} QMC::EnumIsScoped", first ? "" : " |");
        } else {
...
void Generator::generateStaticMetacall()
{
    ...
    if (!cdef->constructorList.isEmpty()) {
        ...
        moc_print(out, "    if (_c == QMetaObject::ConstructInPlace) {{\n        switch (_id) {{\n");
        for (int i = 0; i < cdef->constructorList.size(); ++i) {
            const auto &f = cdef->constructorList.at(i);
