...
template <>
struct std::formatter<QByteArray> : std::formatterstd::string_view {
...
static int nameToBuiltinType(const QByteArray &name) {
  ...
  if (const QtPrivate::QMetaTypeInterface *iface =
          QMetaType::fromName(name).iface())
...
#define RETURN_METATYPENAME_STRING(MetaTypeName, MetaTypeId, RealType)
  case QMetaType::MetaTypeName:
    return #MetaTypeName;
    switch (type) { QT_FOR_EACH_STATIC_TYPE(RETURN_METATYPENAME_STRING) }
#undef RETURN_METATYPENAME_STRING
    return nullptr;
}

Generator::Generator(
    Moc *moc, const ClassDef *classDef, const QList<QByteArray> &metaTypesArg,
    const QHash<QByteArray, QByteArray> &knownQObjectClassesArg,
    const QHash<QByteArray, QByteArray> &knownGadgetsArg,
    const QHash<QByteArray, QByteArray> &hashesArg, FILE *outfile,
    bool requireCompleteTypesArg)
    : parser(moc),
      out(outfile),
      cdef(classDef),
      metaTypes(metaTypesArg),
      knownQObjectClasses(knownQObjectClassesArg),
      knownGadgets(knownGadgetsArg),
      hashes(hashesArg),
...
static inline qsizetype lengthOfEscapeSequence(const QByteArray &s,
                                               qsizetype i) {
...
#undef STREAM_SMART_POINTER
  ...
  for (const QByteArray &smartPointer : smartPointers) {
    ...
    if (propertyType.startsWith(ba) && !propertyType.endsWith("&"))
      return knownQObjectClasses.contains(
          propertyType.mid(smartPointer.size() + 1,
                           propertyType.size() - smartPointer.size() - 1 - 1));
  }
...
#undef STREAM_1ARG_TEMPLATE
  ...
  for (const QByteArray &oneArgTemplateType : oneArgTemplates) {
    ...
    if (propertyType.startsWith(ba) && propertyType.endsWith(">")) {
      const qsizetype argumentSize =
          propertyType.size() - ba.size() - 1 -
          (propertyType.at(propertyType.size() - 2) == ' ' ? 1 : 0);
      const QByteArray templateArg =
          propertyType.sliced(ba.size(), argumentSize);
...
static QByteArray generateQualifiedClassNameIdentifier(
    const QByteArray &identifier) {
  QByteArray qualifiedClassNameIdentifier = "ZN";
  for (const auto scope : qTokenize(QLatin1StringView(identifier),
...
void Generator::generateCode() {
  ...
  const bool requireCompleteness =
      requireCompleteTypes || cdef->requireCompleteMethodTypes;
  bool hasStaticMetaCall =
      (cdef->hasQObject || !cdef->methodList.isEmpty() ||
       !cdef->propertyList.isEmpty() || !cdef->constructorList.isEmpty());
  if (parser->activeQtMode) hasStaticMetaCall = false;
  const QByteArray qualifiedClassNameIdentifier =
      generateQualifiedClassNameIdentifier(cdef->qualified);
  const char ownType = !cdef->hasQNamespace ? cdef->classname.data() : "void";
  moc_print(
      out,
      "namespace {{\nstruct qt_meta_tag_{}t {{}};\n}} // unnamed namespace\n\n",
      qualifiedClassNameIdentifier);
  moc_print(out,
            "template <> constexpr inline auto "
            "{}::qt_create_metaobjectdata<qt_meta_tag{}t>()\n{{\n namespace "
            "QMC = QtMocConstants;\n",
            cdef->qualified, qualifiedClassNameIdentifier);
  moc_print(
      out,
      " static constexpr QtMocHelpers::StringRefStorage qt_stringData {{\n");
...
moc_print(out, """);
  ...
  if (hashIdx == -1)
    moc_print(out, " static constexpr auto qt_metaObjectHashIndex = ~0u;\n");
  else
    moc_print(out,
              " static constexpr auto qt_metaObjectHashIndex = uint32_t({});\n",
              hashIdx);
  ...
  if (isConstructible || !cdef->classInfoList.isEmpty()) {
    ...
    if (!cdef->classInfoList.isEmpty()) {
      moc_print(out,
                " static constexpr QtMocHelpers::ClassInfos qt_classinfo({{\n");
      addClassInfos();
      moc_print(out, " }});\n");
  ...
  if (!requireCompleteness)
    tagType = "qt_meta_tag" + qualifiedClassNameIdentifier + "t";
  moc_print(
      out,
      " return QtMocHelpers::metaObjectData<{}, {}>( {}, qt_stringData,\n "
  ...
  if (cdef->hasQNamespace) {
    metaVarNameSuffix = '' + qualifiedClassNameIdentifier;
    moc_print(out, R"(
static constexpr auto qt_staticMetaObjectContent{0} =
{1}::qt_create_metaobjectdata<qt_meta_tag{0}t>();
...
qt_staticMetaObjectContent{0}.relocatingData;
    ...
    metaVarNameSuffix = "<qt_meta_tag" + qualifiedClassNameIdentifier + "_t>";
  }
  ...
  for (const PropertyDef &p : std::as_const(cdef->propertyList)) {
    ...
    do {
      ...
      QByteArray currentScope = thisScope.isEmpty()
                                    ? unqualifiedScope
                                    : thisScope + "::" + unqualifiedScope;
      scopeIt = knownExtraMetaObject.constFind(currentScope);
    } while (!thisScope.isEmpty() &&
             scopeIt == knownExtraMetaObject.constEnd());
  ...
  for (auto it = cdef->enumDeclarations.keyBegin(),
            end = cdef->enumDeclarations.keyEnd();
       it != end; ++it) {
    const QByteArray &enumKey = *it;
    const qsizetype s = enumKey.lastIndexOf("::");
    if (s > 0) {
      QByteArray scope = enumKey.left(s);
      if (scope != "Qt" && !qualifiedNameEquals(cdef->qualified, scope) &&
          !extraList.contains(scope))
        extraList += scope;
    }
  }
  if (!extraList.isEmpty()) {
    moc_print(out,
              "Q_CONSTINIT static const QMetaObject::SuperData "
              "qt_meta_extradata_{}[] = {{\n",
              qualifiedClassNameIdentifier);
    for (const QByteArray &ba : std::as_const(extraList))
      moc_print(out, " QMetaObject::SuperData::link<{}::staticMetaObject>(),\n",
                ba);
  ...
  if (isQObject)
    moc_print(out, " nullptr,\n");
  else if (cdef->superclassList.size() && !cdef->hasQGadget &&
           !cdef->hasQNamespace)
    moc_print(out, " QMetaObject::SuperData::link<{}::staticMetaObject>(),\n",
              purestSuperClass);
  else if (cdef->superclassList.size())
    moc_print(out, " QtPrivate::MetaObjectForType<{}>::value,\n",
              purestSuperClass);
  else
    moc_print(out, " nullptr,\n");
  moc_print(out,
            " qt_staticMetaObjectStaticContent{0}.stringdata,\n "
  ...
  if (extraList.isEmpty())
    ...
    moc_print(out, " qt_meta_extradata_{},\n", qualifiedClassNameIdentifier);
  moc_print(out,
  ...
  moc_print(out,
            ...
            "return nullptr;\n if (!strcmp(_clname, "
            "qt_staticMetaObjectStaticContent{}.strings))\n return "
  ...
  for (const QListClassDef::Interface &iface :
       std::as_const(cdef->interfaceList)) {
    for (qsizetype j = 0; j < iface.size(); ++j) {
      moc_print(out, " if (!strcmp(_clname, {}))\n return ",
                iface.at(j).interfaceId);
      for (qsizetype k = j; k >= 0; --k)
        moc_print(out, "static_cast< {}>(", iface.at(k).className);
      moc_print(out, "this{});\n", QByteArray(j + 1, ')'));
    }
  }
  if (!purestSuperClass.isEmpty() && !isQObject)
    moc_print(out, " return {}::qt_metacast(clname);\n", purestSuperClass);
  else
    moc_print(out, " return nullptr;\n");
  moc_print(out, "}}\n");
  if (parser->activeQtMode) return;
  generateMetacall();
  for (int i = 0; i < int(cdef->signalList.size()); ++i)
    generateSignal(&cdef->signalList.at(i), i);
  generatePluginMetaData();
  if (!cdef->nonClassSignalList.isEmpty()) {
    moc_print(out, "namespace CheckNotifySignalValidity{} {{\n",
              qualifiedClassNameIdentifier);
    for (const QByteArray &nonClassSignal :
      ...
      moc_print(
          out,
          "template<typename T> using has_nullary_{0} = "
          "decltype(std::declval<T>().{0}());\ntemplate<typename T> using "
          "has_unary_{0} = "
          "decltype(std::declval<T>().{0}(std::declval<{1}>()));\nstatic_"
          "assert(qxp::is_detected_v<has_nullary_{0}, {2}> || "
          "qxp::is_detected_v<has_unary_{0}, {2}>,\n " NOTIFY signal{
              0} does not exist in class(or is private in its parent) ");\n",
...
void Generator::addFunctions(const QList<FunctionDef> &list,
                             const char *functype) {
  for (const FunctionDef &f : list) {
    if (!f.isConstructor) moc_print(out, " // {} '{}'\n", functype, f.name);
    moc_print(out, " QtMocHelpers::{}{}Data<",
              f.revision > 0 ? "Revisioned" : "", functype);
    if (f.isConstructor)
      moc_print(out, "Constructor(");
    else
      moc_print(out, "{}(", disambiguatedTypeName(f.type.name));
    const char *comma = "";
    ...
    if (f.isConstructor)
      ...
      moc_print(out, "){}>({}, {}, ", f.isConst ? " const" : "",
                nIdx == -1 ? "~0u" : std::format("uint32_t({})", nIdx),
                tIdx == -1 ? "~0u" : std::format("uint32_t({})", tIdx));
    if (f.access == FunctionDef::Private)
      moc_print(out, "QMC::AccessPrivate");
    else if (f.access == FunctionDef::Public)
      moc_print(out, "QMC::AccessPublic");
    else if (f.access == FunctionDef::Protected)
      moc_print(out, "QMC::AccessProtected");
    ...
    if (f.arguments.isEmpty())
      ...
      for (qsizetype i = 0; i < f.arguments.size(); ++i) {
        ...
        moc_print(out, ", {} }},",
                  aIdx == -1 ? "0xFFFFu" : std::format("uint16_t({})", aIdx));
      }
      moc_print(out, "\n }}}}),\n");
    }
  }
...
void Generator::generateTypeInfo(const QByteArray &typeName,
                                 bool allowEmptyName) {
  ...
  if (it != typeCache.end()) [[likely]] {
    ...
    if (info.isBuiltin) {
      if (info.valueString)
        moc_print(out, "QMetaType::{}", info.valueString);
      else
        moc_print(out, "{:4d}", info.builtinType);
      return;
    }
  }
  ...
  if (idx == -1)
    moc_print(out, "0x{:08x} | 0xFFFFu", unresolved);
  else
    moc_print(out, "0x{:08x} | uint16_t({})", unresolved, idx);
}
...
void Generator::addProperties() {
  for (const PropertyDef &p : std::as_const(cdef->propertyList)) {
    moc_print(
        out,
        " // property '{}'\n QtMocHelpers::PropertyData<{}{}>(uint32_t({}), ",
        p.name, cxxTypeTag(p.typeTag), disambiguatedTypeName(p.type, p.typeTag),
        stridx(p.name));
    generateTypeInfo(p.type);
    moc_print(out, ",");
    const char *separator = "";
    auto addFlag = [this, &separator](const char *text) {
      moc_print(out, "{} QMC::{}", separator, text);
      separator = " |";
    };
    ...
    if (notifyId != -1 || p.revision > 0) {
      ...
      if (p.notifyId < -1) {
        ...
        moc_print(out, "0x{:08x} | uint32_t({})", unresolved,
                  int(strings.indexOf(p.notify)));
      } else if (notifyId == -1)
        moc_print(out, "~0u");
      else
        moc_print(out, "uint32_t({})", notifyId);
...
void Generator::addEnums() {
  for (const EnumDef &e : std::as_const(cdef->enumList)) {
    ...
    moc_print(out, " // {} '{}'\n", e.flags & EnumIsFlag ? "flag" : "enum",
              e.name);
    moc_print(out, " QtMocHelpers::EnumData<{}>(",
              disambiguatedTypeName(e.name));
    moc_print(out, "{}, {},",
              nIdx == -1 ? "~0u" : std::format("uint32_t({})", nIdx),
    ...
    for (const QByteArray &val : e.values) {
      ...
      moc_print(out, " {{ {}, std::to_underlying({}::{}) }},\n",
                vIdx == -1 ? "~0u" : std::format("uint32_t({})", vIdx), prefix,
                val);
    }
    moc_print(out, " }}),\n");
  }
...
void Generator::generateMetacall() {
  ...
  if (methodList.size()) {
    ...
    if (methodsWithAutomaticTypesHelper(methodList).isEmpty())
      moc_print(out, " *reinterpret_cast<QMetaType *>(_a[0]) = QMetaType();\n");
    else
      moc_print(out, " qt_static_metacall(this, _c, _id, _a);\n");
    moc_print(out, " }}\n _id -= {};\n }}\n", methodList.size());
  }
...
QMultiMap<QByteArray, int> Generator::automaticPropertyMetaTypesHelper() {
  ...
  for (int i = 0; i < int(cdef->propertyList.size()); ++i) {
    ...
    if (registerableMetaType(propertyType) && !isBuiltinType(propertyType))
      automaticPropertyMetaTypes.insert(cxxTypeTag(p.typeTag) + propertyType,
                                        i);
  }
...
Generator::methodsWithAutomaticTypesHelper(
...
void Generator::generateStaticMetacall() {
  moc_print(out,
            "void {}::qt_static_metacall(QObject _o, QMetaObject::Call _c, int "
            "_id, void _a)\n{{\n",
            cdef->qualified);
  enum UsedArgs { UsedT = 1, UsedC = 2, UsedId = 4, UsedA = 8 };
  uint usedArgs = 0;
  if (cdef->hasQObject)
    moc_print(out, " auto *_t = static_cast<{} *>(_o);\n", cdef->classname);
  else
    moc_print(out, " auto *_t = reinterpret_cast<{} *>(_o);\n",
              cdef->classname);
  if (!cdef->constructorList.isEmpty()) {
    ...
    for (int i = 0; i < cdef->constructorList.size(); ++i) {
      ...
      moc_print(out, " case {}: {{ {} *_r = new {}(", i, cdef->classname,
                cdef->classname);
      for (int j = 0; j < f.arguments.size(); ++j)
        moc_print(
            out, "{}reinterpret_cast<{}>(_a[{}])", j == 0 ? "" : ",",
            disambiguatedTypeNameForCast(f.arguments.at(j).normalizedType),
            j + 1);
      moc_print(out,
  ...
  if (!methodList.isEmpty()) {
    ...
    if (methodList.size() <= 16) {
      ...
      for (int i = 0; i < methodList.size(); ++i) {
        ...
        if (f.normalizedType == "void" && !f.isRawSlot && f.arguments.isEmpty())
          moc_print(out, "(void)a; ");
        if (f.isStatic) moc_print(out, "(void)t; ");
        if (f.normalizedType != "void") moc_print(out, "if (auto r = ");
        moc_print(out, "{}{}(",
                  f.isStatic ? std::format("{}::", cdef->classname) : "t->",
                  f.name);
        if (f.isRawSlot) {
          moc_print(out, "QMethodRawArguments{{a}}");
          usedArgs |= UsedA;
        } else {
          for (int j = 0; j < f.arguments.size(); ++j) {
            moc_print(
                out, "{}reinterpret_cast<{}>(a[{}])", j == 0 ? "" : ",",
                disambiguatedTypeNameForCast(f.arguments.at(j).normalizedType),
                j + 1);
            usedArgs |= UsedA;
          }
        ...
        if (f.normalizedType != "void")
          moc_print(out,
                    "if (a[0]) reinterpret_cast<{}>(a[0]) = std::move(r); ",
                    disambiguatedTypeName(noRef(f.normalizedType)));
        moc_print(out, "}}");
      ...
      for (int i = 0; i < methodList.size(); ++i) {
        ...
        if (f.normalizedType != "void")
          moc_print(out, "{{ {} _r = ",
                    disambiguatedTypeName(noRef(f.normalizedType)));
        moc_print(out, "{}{}(",
                  f.isStatic ? std::format("{}::", cdef->classname) : "_t->",
                  f.name);
        if (f.isRawSlot) {
          moc_print(out, "QMethodRawArguments{{_a}}");
          usedArgs |= UsedA;
        } else {
          for (int j = 0; j < f.arguments.size(); ++j) {
            moc_print(
        ...
        if (f.normalizedType != "void")
          moc_print(
              out,
              "if (_a[0]) reinterpret_cast<{}>(_a[0]) = std::move(_r); }} ",
              disambiguatedTypeName(noRef(f.normalizedType)));
        moc_print(out, "break;\n");
      }
      moc_print(out, " default: std::unreachable();\n }}\n");
...
void Generator::generateSignal(const FunctionDef *def, int index) {
  ...
  moc_print(out, "\n// SIGNAL {}\n{} {}::{}(", index, def->type.name,
            cdef->qualified, def->name);
  QByteArray thisPtr =
      def->isConst ? std::format("const_cast<{} *>(this)", cdef->qualified)
                   : "this";
  for (int i = 0; i < def->arguments.size(); ++i) {
    moc_print(out, "{}{} _t{}{}", i == 0 ? "" : ", ",
              def->arguments.at(i).type.name, i + 1,
              def->arguments.at(i).rightType);
  }
  moc_print(out, "){}\n{{\n", def->isConst ? " const" : "");
  if (def->type.name != "void")
    moc_print(out, " {} _t0{{}};\n", noRef(def->normalizedType));
  moc_print(out, " QMetaObject::activate<{}>( {}, &staticMetaObject, {}, {}",
            def->normalizedType, thisPtr, index,
            def->normalizedType == "void" ? "nullptr" : "std::addressof(_t0)");
  for (int i = 0; i < def->arguments.size(); ++i)
    moc_print(out, ", _t{}", i + 1);
  moc_print(out, ");\n");
...
static CborError jsonValueToCbor(CborEncoder *parent, const QJsonValue &v) {
  switch (v.type()) {
    ...
    case QJsonValue::Double: {
      double d = v.toDouble();
      if (d == std::floor(d) &&
          std::abs(d) <= (Q_INT64_C(1) << std::numeric_limits<double>::digits))
...
void Generator::generatePluginMetaData() {
  ...
  auto outputCborData = this {
    ...
    cbor_encode_text_string(&map, cdef->pluginData.iid.constData(),
                            cdef->pluginData.iid.size());
    dev.nextItem("" className "");
    cbor_encode_int(&map, int(QtPluginMetaDataKeys::ClassName));
    cbor_encode_text_string(&map, cdef->classname.constData(),
                            cdef->classname.size());
    ...
    if (!cdef->pluginData.uri.isEmpty()) {
      ...
      cbor_encode_text_string(&map, cdef->pluginData.uri.constData(),
                              cdef->pluginData.uri.size());
    }
    for (auto it = cdef->pluginData.metaArgs.cbegin(),
              end = cdef->pluginData.metaArgs.cend();
         it != end; ++it) {
  ...
  for (qsizetype pos = cdef->qualified.indexOf("::"); pos != -1;
       pos = cdef->qualified.indexOf("::", pos + 2))
    moc_print(out, "using namespace {};\n", cdef->qualified.left(pos));
  moc_print(out,
            "\n#ifdef QT_MOC_EXPORT_PLUGIN_V2\nstatic constexpr unsigned char "
            "qt_pluginMetaDataV2_{}[] = {{",
            cdef->classname);
  outputCborData();
  moc_print(
      out,
      ...
      "static constexpr unsigned char qt_pluginMetaData_{}[] = {{\n 'Q', 'T', "
      "'M', 'E', 'T', 'A', 'D', 'A', 'T', 'A', ' ', '!',\n 0, "
      "QT_VERSION_MAJOR, QT_VERSION_MINOR, qPluginArchRequirements(),",
      cdef->qualified, cdef->classname, cdef->classname, cdef->classname);
  outputCborData();
